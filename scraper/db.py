    ...
    with _get_conn() as conn:
        conn.execute(
            ...
            UPDATE jobs SET
                ...
                job_summary = ?, top_qualifications = ?, job_category = ?
            WHERE id = ?
            """,
            (
                score, fit_category,
                json.dumps(key_matches), json.dumps(key_gaps), rationale,
                job_summary, json.dumps(top_qualifications), job_category,
                job_id,
            ),
        )
    ...
    with _get_conn() as conn:
        rows = conn.execute(
            ...
            SELECT id, source, url, title, company, location, date_posted, date_found,
                   ...
                   status, feedback, connection_notes
            FROM jobs
            WHERE (score IS NULL OR score >= ?) AND status != 'archived'
...
def touch_last_seen(url: str) -> None:
    """Update last_seen_date for a job URL (called each time it appears in a scrape)."""
    job_id = _job_id(url)
    with _get_conn() as conn:
        conn.execute(
...
def archive_stale_jobs() -> int:
    """Mark jobs as archived if they are >14 days old and last seen >7 days ago.

    Only archives jobs where last_seen_date has been set at least once
    (i.e., appeared in a scrape run after this feature was deployed).
    Jobs with NULL last_seen_date are left untouched — we don't know their age.
    Jobs with status 'applied', 'interested', or 'skip' are never auto-archived.
    Returns count of archived jobs.
    """
    cutoff_found = (datetime.utcnow() - timedelta(days=14)).isoformat()
    cutoff_seen = (datetime.utcnow() - timedelta(days=7)).isoformat()
    with _get_conn() as conn:
        result = conn.execute(
            """
            ...
            WHERE date_found < ?
              ...
              AND last_seen_date < ?
              AND status NOT IN ('applied', 'interested', 'archived', 'skip')
            """,
            (cutoff_found, cutoff_seen),
        )
        return result.rowcount


def get_new_high_score_jobs(min_score: int = 80) -> list[dict]:
    """Return newly-scored jobs at or above min_score that haven't had an alert sent."""
    with _get_conn() as conn:
        rows = conn.execute(
            """
            SELECT id, title, company, location, url, score, fit_category,
                   job_summary, rationale, job_category
            FROM jobs
            WHERE score >= ? AND alert_sent_at IS NULL AND status != 'archived'
            ORDER BY score DESC
            """,
            (min_score,),
        ).fetchall()
...
def mark_alerts_sent(job_ids: list[str]) -> None:
    """Record that alert emails have been sent for the given job IDs."""
    if not job_ids:
        return
    ts = datetime.utcnow().isoformat()
    with _get_conn() as conn:
        conn.executemany(
            "UPDATE jobs SET alert_sent_at = ? WHERE id = ?",
            [(ts, jid) for jid in job_ids],
        )


def get_jobs_for_digest(days: int = 7, min_score: int = 35) -> list[dict]:
    """Return new jobs for the weekly digest: found in last N days, not yet digested."""
    cutoff = (datetime.utcnow() - timedelta(days=days)).isoformat()
    with _get_conn() as conn:
        rows = conn.execute(
            """
            SELECT id, title, company, location, url, score, fit_category,
                   job_summary, rationale, job_category, date_found
            FROM jobs
            WHERE date_found >= ? AND score >= ? AND digest_sent_at IS NULL
              AND status != 'archived'
            ORDER BY score DESC
            """,
            (cutoff, min_score),
...
def mark_digest_sent(job_ids: list[str]) -> None:
    """Record that the weekly digest has been sent for the given job IDs."""
    if not job_ids:
        return
    ts = datetime.utcnow().isoformat()
    with _get_conn() as conn:
        conn.executemany(
            "UPDATE jobs SET digest_sent_at = ? WHERE id = ?",
            [(ts, jid) for jid in job_ids],
        )
